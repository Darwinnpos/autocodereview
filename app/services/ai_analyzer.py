...
- **severity判断标准（严格遵守）**：
  * **error**: 会导致程序崩溃、运行失败、数据损坏、安全漏洞的严重问题（如：语法错误、未定义变量、空指针、SQL注入、XSS漏洞、无限循环、内存泄漏等）
  * **warning**: 可能导致bug、性能问题或代码质量下降的潜在问题（如：未使用的变量、类型不匹配、缺少错误处理、逻辑漏洞、性能瓶颈等）
  * **info**: 不影响功能但可以改进的代码风格或最佳实践建议（如：命名规范、注释建议、代码格式等）
